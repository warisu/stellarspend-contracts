...
fn print_metrics(name: &str, cpu: u64, mem: u64) {
    extern crate std;
    std::println!("[BENCHMARK] {} | CPU: {} | Memory: {} bytes", name, cpu, mem);
}
...
fn benchmark_budget_creation() {
    ...
    assert!(cpu_used < 5_000_000, "Budget creation CPU regression: {}", cpu_used);
}
...
fn benchmark_spending_validation() {
    ...
    print_metrics("Spending Validation (spend_from_category)", cpu_used, mem_used);
    
    assert!(cpu_used < 5_000_000, "Spending validation CPU regression: {}", cpu_used);
}
...
fn benchmark_goal_creation() {
    ...
    print_metrics("Goal Creation (batch_set_savings_goals n=1)", cpu_used, mem_used);
    
    assert!(cpu_used < 8_000_000, "Goal creation CPU regression: {}", cpu_used);
}
...
fn benchmark_goal_contribution() {
    ...
    assert!(cpu_used < 5_000_000, "Goal contribution CPU regression: {}", cpu_used);
}
...
fn benchmark_goal_withdrawal() {
    ...
    assert!(cpu_used < 5_000_000, "Goal withdrawal CPU regression: {}", cpu_used);
}
...
fn benchmark_event_emission_overhead() {
    ...
    print_metrics("Event Emission Overhead (approx from update_budget)", cpu_used, mem_used);
    
    assert!(cpu_used < 2_000_000, "Event emission overhead regression: {}", cpu_used);
